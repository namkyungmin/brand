...
<body>
	...
	<form method="post" name="mainForm" id="mainForm" action="/membership/find-id/result">
	</c:if>
	<c:if test="${param.mode=='find-pw'}">
	<form method="post" name="mainForm" id="mainForm" action="/membership/find-password/result">
		<input type="hidden" name="userId" id="userId" value="${param.userId }" >
	</c:if>
	<c:if test="${param.mode==''}">	
	<form method="post" name="mainForm" id="mainForm" action="/membership/join/form">
	</c:if>
	<c:if test="${param.mode==null}">	
	<form method="post" name="mainForm" id="mainForm" action="/membership/join/form">
	</c:if>	
	<c:if test="${param.mode=='join'}">	
	<form method="post" name="mainForm" id="mainForm" action="/membership/join/form">
	</c:if>	
		<input type="hidden" name="certType" value="10" >
		<input type="hidden" name="sDupInfo" value="<%=sDupInfo %>" >
		<input type="hidden" name="sName" value="<%=sName %>" >
	</form>
	<script>document.mainForm.submit();window.close();</script>
	<%} %>
	<!--
	<%=sRtnMsg%> 
	 -->	
